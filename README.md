...
> **Wave 8 placeholder.** Quality will be measured with real providers before any score is published.

| Metric | Result | Embedder | LLM | Judge | n | Date |
| --- | --- | --- | --- | --- | --- | --- |
| Retrieval and answer quality | Pending wave 8 | Real provider, TBD | Real provider, TBD | Calibrated judge, TBD | TBD | TBD |

**Provenance is part of the metric.** Every published number must carry the embedder, LLM, judge, sample size, and measurement date in the same table. Runs using the deterministic fake providers are plumbing checks and never quality claims.
